...
CSV_TO_EXECUTE = '''SELECT insurance_policy.id AS Policy_number,
...
def create_w(request, view_id):
    if Register.objects.filter(id=view_id).exists() and Register.objects.get(id=view_id).sender == request.sender:
        return create_response([view_id], user_friendly=True)
    else:
...
def get_post_params(request, policy):
...
def create_user(request, policy):
    post_params = get_post_params(request, policy)
    try:
...
def dashboard(request):
    ...
    for current_id, policy_id in enumerate(insurancy_policy_info):
        ...
        try:
            ...
            if policy_id.payment_id and policy_id.payment_id.status < 0:
                ...
                logger.error(
                    "An error has occurred while trying to get insurane period.\
                    Reason: " + str(error))
    ...
    for stock_exchange in stock_exchange_tags:
        ...
        context = {
            ...
            # ToDo: Check if user already referral pattern
            'is_referral': False
        }
